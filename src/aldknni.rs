    ...
    let (vec_idx, vec_corr) = if (vec_idx.len() < *min_l_loci) || (*min_loci_corr == 0.0) {
        ...
        indices.sort_by(
            |&a, &b| match (vec_corr[b].is_nan(), vec_corr[a].is_nan()) {
                ...
                (false, false) => vec_corr[b].partial_cmp(&vec_corr[a]).expect("Error comparing a pair of loci correlations as both as f64::NAN."),
            },
        );
...
fn find_k_nearest_neighbours(
    ...
    indices.sort_by(
        |&a, &b| match (distances[a].is_nan(), distances[b].is_nan()) {
            ...
            (true, false) => Ordering::Greater,
            (false, true) => Ordering::Less,
            (false, false) => distances[a].partial_cmp(&distances[b]).expect("Error comparing a pair fo distances as both are f64::NAN."),
        },
    );
