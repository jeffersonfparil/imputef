...
fn find_k_nearest_neighbours(
    ...
    intercept_and_allele_frequencies: &Array2<f64>,
) -> io::Result<(Vec<usize>, Vec<f64>, Array1<f64>)> {
// ) -> io::Result<(Vec<usize>, Vec<f64>, Array2<f64>)> {
    ...
    let mut freq: Array1<f64> =
        Array1::from_elem(idx.len(), f64::NAN);
    for i in 0..idx.len() {
        freq[i] = intercept_and_allele_frequencies[(idx[i], idx_allele_ini)];
    }
    Ok((idx, dist, freq))
    // // Extract non-missing allele frequencies at the locus requiring imputation of the k-nearest neighbours
        ...
        for j in 0..(loci_idx.len()-1) {
            ...
            for i in 0..n {
                if self_clone.intercept_and_allele_frequencies[(i, idx_locus_ini)].is_nan() {
                    let sum = self.intercept_and_allele_frequencies.slice(s![i, idx_locus_ini..idx_locus_fin]).sum();
                    if sum != 1.0 {
                        if (idx_locus_fin - idx_locus_ini) == 2 {
                            self.intercept_and_allele_frequencies[(i, idx_locus_ini+1)] = 1.00 - self.intercept_and_allele_frequencies[(i, idx_locus_ini)]
                        } else {
                            for k in idx_locus_ini..idx_locus_fin {
                                self.intercept_and_allele_frequencies[(i, k)] = self.intercept_and_allele_frequencies[(i, k)] / sum;
                            }
                        }
                    }
...
pub fn impute_aldknni(
    ...
    let (loci_idx, loci_chr, _loci_pos) = genotypes_and_phenotypes.count_loci().expect("Error calling count_loci() method within adaptive_ld_knn_imputation() method for GenotypesAndPhenotypes struct.");
...
mod tests {
    ...
    fn test_aldknni() {
        ...
        let (loci_idx, loci_chr, _loci_pos) = frequencies_and_phenotypes.count_loci().unwrap();
